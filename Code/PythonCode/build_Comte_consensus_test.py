...
import build_Comte_consensus as bcc
...
class bccTest(unittest.TestCase):
    def adjlist_file_setup(self):
        ...
        pd_1.to_csv('bcc_test/test_1-10000-adjlist.csv', index=False)
        test_2 = {'Child': ['a', 'b', 'b', 'c', 'c'],
                  'Parent': ['a', 'b', 'c', 'c', 'a']}
        pd_2 = pd.DataFrame(test_2)
        pd_2.to_csv('bcc_test/test_2-500-adjlist.csv', index=False)
        return path, pd_1, pd_2


    def adjlist_file_teardown(self):
        os.remove('bcc_test/test_1-10000-adjlist.csv')
        os.remove('bcc_test/test_2-500-adjlist.csv')
        if os.path.isfile('bcc_test/out.tmp'):
            os.remove('bcc_test/out.tmp')
        os.rmdir('bcc_test')


    def test_get_adjlist_scores(self):
        path, __, __ = self.adjlist_file_setup()
        adjlist_scores = bcc.get_adjlist_scores(path)
        fnames = set(['bcc_test/test_1-10000-adjlist.csv',
                      'bcc_test/test_2-500-adjlist.csv'])
    ...
    def test_top_n_adjlists(self):
        ...
        adjlist_scores = pd.DataFrame(
            {'score': [10000, 500],
             'fname': ['bcc_test/test_1-10000-adjlist.csv',
                        'bcc_test/test_2-500-adjlist.csv']})
        adjlists = bcc.top_n_adjlists(adjlist_scores)
        self.assertEqual(len(adjlists), 2)
        assert pd_1.equals(adjlists[0])
        assert pd_2.equals(adjlists[1])
        adjlists = bcc.top_n_adjlists(adjlist_scores, n=1)
    ...
    def test_add_adjlist_to_dict(self):
        __, pd_1, pd_2 = self.adjlist_file_setup()
        fulldict = bcc.add_adjlist_to_dict(pd_1)
        assert max(fulldict.values()) == 1
        assert len(fulldict) == 4
        assert fulldict[('b', 'c')] == 1
        fulldict = bcc.add_adjlist_to_dict(pd_2, full_adjlist_dict=fulldict)
    ...
    def test_consensus_from_full_adjlist(self):
        ...
        fulldict[('a', 'b')] = 2
        fulldict[('b', 'b')] = 1
        consensus = bcc.consensus_from_full_adjlist(fulldict, 4, cutoff=.5)
        ...
        consensus = bcc.consensus_from_full_adjlist(fulldict, 4, cutoff=.75)
        self.assertEqual(consensus.shape, (1,3))


    def test_compare_consensus_best(self):
        __, pd_1, pd_2 = self.adjlist_file_setup()
        out = bcc.compare_consensus_best(pd_1, pd_2)
    ...
    def test_build_Comte_consensus(self):
        __, pd_1,pd_2 = self.adjlist_file_setup()
        outfile = 'bcc_test/out.tmp'
        [consensus, adjlists] = bcc.build_Comte_consensus('bcc_test',
                                                          outfile,
                                                          n=10,
                                                          cutoff=.6)
